...
export function RelayAgentConfig({
  ...
  const [name, setName] = useState(profile?.name ?? 'Relay Agent')
  const [relayHost, setRelayHost] = useState(String(cfg.relay_host ?? '127.0.0.1'))
  const [relayPort, setRelayPort] = useState(String(cfg.relay_port ?? '9999'))
  ...
  const mut = useMutation({
    mutationFn: () => {
      const config = {
        ...
        relay_port: parseInt(relayPort, 10),
      ...
      return profile
        ...
        : connectivityApi.createProfile({ name, mode: 'RELAY_AGENT', config, notes: notes || undefined })
    },
    onSuccess: (p) => { qc.invalidateQueries({ queryKey: ['connectivity-profiles'] }); onSaved(p) },
  })

  return (
    <PanelShell title="RELAY AGENT [BETA]" color="#f87171">
      <div className="p-3 rounded-xl bg-red-500/5 border border-red-500/10 text-xs font-mono text-red-400/60 space-y-1">
        <div className="flex items-center gap-2">
          <Shield className="w-3.5 h-3.5 flex-shrink-0" />
          <span>Deploy a lightweight relay agent on your jump host. Agent calls back to AdByG0d — no inbound firewall rules needed.</span>
        </div>
        <div className="text-red-400/40">STATUS: Coming in v5.0 — configuration is stored and reserved.</div>
      </div>
      <ConfigField label="Profile Name">
        <CInput value={name} onChange={setName} placeholder="e.g. Relay via DMZ Host" />
      </ConfigField>
      <div className="grid grid-cols-2 gap-3">
        <ConfigField label="Relay Host" hint="jump host IP where agent runs">
          <CInput value={relayHost} onChange={setRelayHost} placeholder="10.10.0.5" />
        </ConfigField>
        <ConfigField label="SOCKS5 Port">
          <CInput value={relayPort} onChange={setRelayPort} type="number" />
        </ConfigField>
      </div>
      ...
      <SaveBar onSave={() => mut.mutate()} onCancel={onCancel} saving={mut.isPending} label="RESERVE PROFILE" error={saveErrorMessage(mut.error)} />
      <div className="mt-4 p-4 rounded-xl border border-red-500/10 bg-red-500/3">
        <div className="text-xs font-mono text-red-400/50 uppercase tracking-widest mb-3">DEPLOYMENT PREVIEW</div>
        <pre className="text-[11px] font-mono text-red-300/40 whitespace-pre-wrap">{`# On jump host
wget https://github.com/adbygod/relay-agent/releases/latest/relay-agent-linux-amd64
chmod +x relay-agent-linux-amd64
./relay-agent-linux-amd64 --server https://<adbygod-host> --token <token>`}</pre>
      </div>
    </PanelShell>
  )
}
