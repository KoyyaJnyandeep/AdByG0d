...
export function DirectConfig({
  ...
  const [name, setName] = useState(profile?.name ?? 'Direct')
  ...
  const mut = useMutation({
    mutationFn: () => {
      ...
      return profile
        ? connectivityApi.updateProfile(profile.id, { name, config, notes: notes || undefined, is_default: isDefault })
        : connectivityApi.createProfile({ name, mode: 'DIRECT', config, notes: notes || undefined, is_default: isDefault })
    },
    onSuccess: (p) => { qc.invalidateQueries({ queryKey: ['connectivity-profiles'] }); onSaved(p) },
  })

  return (
    <PanelShell title="DIRECT MODE" color="#22d3ee">
      <div className="p-3 rounded-xl bg-cyan-500/5 border border-cyan-500/10 text-xs font-mono text-cyan-500/60">
        No proxy. API connects directly to DC. Requires network route from this host to the Domain Controller.
      </div>
      <ConfigField label="Profile Name">
        <CInput value={name} onChange={setName} placeholder="e.g. Corp DC Direct" />
      </ConfigField>
      <ConfigField label="Notes">
        <CInput value={notes} onChange={setNotes} placeholder="Optional notes" />
      </ConfigField>
      <label className="flex items-center gap-2 cursor-pointer">
        <input type="checkbox" checked={isDefault} onChange={e => setIsDefault(e.target.checked)} className="accent-cyan-400" />
        <span className="text-xs font-mono text-white/50">Set as default profile</span>
      </label>
      {/* Target Details */}
      ...
      <SaveBar
        ...
        error={saveErrorMessage(mut.error)}
      />
    </PanelShell>
  )
}
